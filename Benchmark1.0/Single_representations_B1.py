...
def data_transfomation(features_packed_test, rep):
    Mydir = sorted(glob.glob('Benchmark1.0/First_Model/'+str(rep)+'_*.pkl'))
    x_test_pre = []
    i = 0
    for dir in Mydir:
        print(i, dir)
...
def Second_estimators(features_packed_test, y_test):
    for rep in range(1, 5):
        ...
        for model in (model1, model2, model3, model4, model5):
            ...
            joblib.dump(model, 'Benchmark1.0/Rep_second_model/'+str(i + 1) + '_' + str(rep) + '.h5')
            i += 1
...
def Independent_test(features_packed_ind_test, Ind_Label):
    ...
    for rep in range(1, 5):
        ...
        Mydir = sorted(glob.glob('Benchmark1.0/Rep_second_model/*_'+str(rep)+'.h5'))
        j = 0
        for dir in Mydir:
            print(dir)
            model = joblib.load(dir)
    ...
    res.to_excel('Benchmark1.0/Single_representations_B1.xlsx')
