...
def Load_features():
    ...
    Label = np.concatenate((np.ones([31940], dtype=int), np.zeros([31940], dtype=int)), axis=0)
...
def Base_estimators(features_packed, Label):
    ...
    for i in range(len(features_packed)):
        ...
        for model in (model1, model2, model3, model4, model5):
            ...
            joblib.dump(model, 'ThermoSeq_d1.0/First_Model/'+str(i+1)+'_'+str(j+1)+'.pkl')
            j += 1
...
def Second_estimators(features_packed_test, y_test):
    ...
    for model in (model1, model2, model3, model4, model5):
        ...
        joblib.dump(model, 'ThermoSeq_d1.0/Second_Model/'+str(i + 1) + '_' + '.h5')
        i += 1
...
def Independent_test(features_packed_ind_test, Ind_Label):
    ...
    Mydir = sorted(glob.glob('ThermoSeq_d1.0/Second_Model/*.h5'))
    for dir in Mydir:
        print(dir)
        model = joblib.load(dir)
