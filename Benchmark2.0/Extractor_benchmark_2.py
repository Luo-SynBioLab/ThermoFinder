...
def Load_data():
    print('Data Loading...')
    Sequence = []
    with open('Benchmark2.0/Trainingset.txt', 'r') as myfile:
        for line in myfile:
            if line[0] != '>':
                Sequence.append(line.strip('\n'))
    with open('Benchmark2.0/Testset.txt', 'r') as myfile:
        for line in myfile:
            if line[0] != '>':
                Sequence.append(line.strip('\n'))
...
def Write_features(features_ensemble):
    ...
    for i in range(len(features_ensemble)):
        if i < 1024:
            Mydic['protTrans_'+str(i+1)] = features_ensemble[i]
        elif i < 1024+1536:
            Mydic['CPC_'+str(i+1-1024)] = features_ensemble[i]
        elif i < 1024+1536+1024:
            Mydic['ELMO_'+str(i+1-1024-1536)] = features_ensemble[i]
        else:
            Mydic['CNN_'+str(i+1-1024-1536-1024)] = features_ensemble[i]
    with open('Benchmark2.0/Features_proteome.pkl', 'wb') as f:
        pickle.dump(Mydic, f)
...
if __name__ == '__main__':
    ...
    # Save features
    features_ensemble = np.concatenate((f_protTrans, f_CPC, f_Elmo, f_CNN), axis=1)
